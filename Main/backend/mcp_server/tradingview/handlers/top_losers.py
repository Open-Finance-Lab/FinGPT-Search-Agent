"""Handler for get_top_losers tool."""
...
class GetTopLosersHandler(TradingViewBaseHandler):
    """Handler for get_top_losers tool.

    Returns worst performing assets by exchange and timeframe.
    """

    async def execute(self, ctx: ToolContext) -> List[types.TextContent]:
        """Execute get_top_losers tool.
        ...
        Returns:
            List containing top losers as JSON
        """
        try:
            # Validate inputs
            ...
            if cached:
                logger.info(f"Cache hit for top_losers:{exchange}:{timeframe}")
                return [types.TextContent(type="text", text=cached)]

            logger.info(f"Cache miss for top_losers:{exchange}:{timeframe}")

            # Call TradingView MCP
            response = await self._call_tradingview(
                tool_name="get_top_losers",
                params={
                    "exchange": exchange,
                    "timeframe": timeframe,
                    "limit": limit
            ...
            logger.error(f"Error in get_top_losers: {e}", exc_info=True)
            error_msg = {
                "error": "losers_error",
                "message": str(e),
