"""Handler for get_top_gainers tool."""
...
class GetTopGainersHandler(TradingViewBaseHandler):
    """Handler for get_top_gainers tool.

    Returns top performing assets by exchange and timeframe.
    """

    async def execute(self, ctx: ToolContext) -> List[types.TextContent]:
        """Execute get_top_gainers tool.
        ...
        Returns:
            List containing top gainers as JSON
        """
        try:
            # Validate inputs
            ...
            if cached:
                logger.info(f"Cache hit for top_gainers:{exchange}:{timeframe}")
                return [types.TextContent(type="text", text=cached)]

            logger.info(f"Cache miss for top_gainers:{exchange}:{timeframe}")

            # Call TradingView MCP
            response = await self._call_tradingview(
                tool_name="get_top_gainers",
                params={
                    "exchange": exchange,
                    "timeframe": timeframe,
                    "limit": limit
